...
Three `az` commands are all that's needed to grant the permissions below.
...
I am the DCRI point person for AI agent development on Azure AI Foundry. Each new
...
Currently each project requires 3–5 IT tickets for routine setup. These three permissions
— all narrowly scoped — would eliminate that friction.
